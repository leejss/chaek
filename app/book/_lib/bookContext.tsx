...
import { Book, BookActions, BookContextState, BookDraft, GeminiModel, User } from './types';
...
const defaultUser: User = { id: 'u1', name: 'Author', email: 'author@bookmaker.com' };
...
const BookProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const generateTOC = useCallback(async (sourceText: string) => {
    ...
    setError(null);
    setCurrentBook((prev) => ({ ...prev, sourceText, status: 'generating_toc' }));
    try {
  ...
  const startBookGeneration = useCallback(async (model?: GeminiModel) => {
    if (!currentBook.tableOfContents.length) {
      setError('차례가 없습니다. 먼저 TOC를 생성하세요.');
      return;
    }

    const selectedModel = model || currentBook.selectedModel || GeminiModel.FLASH;
    setCurrentBook((prev) => ({ ...prev, status: 'generating_book', selectedModel }));
    setStreamingContent('');
    setError(null);
  ...
  }, [currentBook.selectedModel, currentBook.sourceText, currentBook.tableOfContents]);
  ...
  const state: BookContextState = useMemo(
    ...
    [books, currentBook, currentUser, error, isAuthenticated, isProcessing, streamingContent],
  );
  ...
  return <BookContext.Provider value={{ state, actions }}>{children}</BookContext.Provider>;
};
