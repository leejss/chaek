...
export type ReadJsonResult = { ok: true; data: unknown } | { ok: false; error: InvalidJsonError };
...
export function parseAndValidateBody<T>(body: unknown, schema: z.ZodSchema<T>): T {
  const result = schema.safeParse(body);
  if (!result.success) {
    console.error("[InvalidRequestBodyError] Invalid request body:", result.error);
...
export function formatDate(
  date: DateInput,
  { formatStr = "PPP", relative = false, addSuffix = true }: FormatDateOptions = {},
): string {
